...
static bool publish_common_info(void) {
  ...
  if (ha_common_info_published == false) {
    ...
    // ---- NEW: publicar todas as temperaturas do Leaf no mesmo documento "info" ----
    const uint8_t n = datalayer_extended.nissanleaf.temperature_sensor_count;
    doc["temperature_sensor_count"] = n;  
    // ---- /NEW ----
...
static bool publish_cell_voltages(void) {
  ...
  if (datalayer.battery.info.number_of_cells != 0u &&
    ...
    JsonArray temps = doc["temperature_sensors"].to<JsonArray>();
    for (uint8_t i = 0; i < 4; ++i) {
      int16_t dC = datalayer_extended.nissanleaf.temperature_sensors_dC[i];
      if (dC == INT16_MIN) continue;           // inválido/não usado
      temps.add(((float)dC) / 10.0f);          // publica em °C
    }
