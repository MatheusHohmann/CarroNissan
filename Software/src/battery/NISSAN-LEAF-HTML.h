...
class NissanLeafHtmlRenderer : public BatteryHtmlRenderer {
 public:
  String get_status_html() {
    ...
    content += "<h4>Temperature sensors (" 
            + String(datalayer_extended.nissanleaf.temperature_sensor_count) 
            + ")</h4>";

    for (uint8_t i = 0; i < 4; ++i) {
      int16_t dC = datalayer_extended.nissanleaf.temperature_sensors_dC[i];
      if (dC == INT16_MIN) continue;
      char buf[32];
      float c = dC / 10.0f;
      snprintf(buf, sizeof(buf), "Sensor %u: %.1f &deg;C", (unsigned)(i + 1), c);
      content += String("<h4>") + String(buf) + String("</h4>");
    }
    return content;
  }
};
