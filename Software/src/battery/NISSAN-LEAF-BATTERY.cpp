...
void NissanLeafBattery::
  ...
  if (LEAF_battery_Type == ZE0_BATTERY) {
    ...
    // ---- NEW: Preencher datalayer estendido com todas as temperaturas individuais ----
  if (datalayer_nissan) {
    datalayer_nissan->temperature_sensor_count = leaf_temp_sensor_count_;
    for (uint8_t i = 0; i < 4; ++i) {
      datalayer_nissan->temperature_sensors_dC[i] = leaf_temp_sensors_dC_[i];
    }
  }
    ...
    case 0x7BB:
      ...
      if (group_7bb == 0x04) {              //Temperatures
        ...
        if (rx_frame.data.u8[0] == 0x22) {  //Third message
          ...
          // ---- NEW: Converter valores RAW para décimos de °C e preencher cache ----
          auto raw_to_dC = [&](uint16_t raw) -> int16_t {
            if (raw == 65535) return INT16_MIN;
            uint16_t f_times10 = Temp_fromRAW_to_F(raw);
            return static_cast<int16_t>(((int32_t)f_times10 - 320) * 5 / 9);
          };

          int16_t dC1 = raw_to_dC(battery_temp_raw_1);
          int16_t dC2 = raw_to_dC(battery_temp_raw_2);
          int16_t dC3 = raw_to_dC(battery_temp_raw_3);
          int16_t dC4 = raw_to_dC(battery_temp_raw_4);

          // Detecta contagem real (3 ou 4 sensores)
          if (battery_temp_raw_3 == 65535) {
            leaf_temp_sensor_count_ = 3;
            leaf_temp_sensors_dC_[0] = dC1;
            leaf_temp_sensors_dC_[1] = dC2;
            leaf_temp_sensors_dC_[2] = dC4;
            leaf_temp_sensors_dC_[3] = INT16_MIN;
          } else {
            leaf_temp_sensor_count_ = 4;
            leaf_temp_sensors_dC_[0] = dC1;
            leaf_temp_sensors_dC_[1] = dC2;
            leaf_temp_sensors_dC_[2] = dC3;
            leaf_temp_sensors_dC_[3] = dC4;
          }
          // ---- /NEW ----
