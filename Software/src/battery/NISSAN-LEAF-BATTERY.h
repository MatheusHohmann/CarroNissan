...
class NissanLeafBattery : public CanBattery {
 ...
 // ---- NEW: cache das temperaturas individuais em décimos de °C e contagem detectada ----
  int16_t leaf_temp_sensors_dC_[4] = {INT16_MIN, INT16_MIN, INT16_MIN, INT16_MIN}; // INT16_MIN = não definido
  uint8_t leaf_temp_sensor_count_ = 0; // 3 ou 4 (conforme o pack)

  // Publicação MQTT (helper)
  void publish_leaf_temps_mqtt_();
